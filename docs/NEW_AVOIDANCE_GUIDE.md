...
# 反应式避障开发记录 / Reactive Avoidance Development Log

## 方案一：动态角度跟踪（已失败 / FAILED）

### 原理

触发时记录障碍物初始角度（`obstacle_initial_angle`），在 MAINTAIN 阶段实时计算障碍物相对车体的角度：

```
obstacle_relative_angle = obstacle_initial_angle - yaw_change_deg
```

例：初始 −15°，左转 +30° → 障碍物相对角 = −15° − 30° = −45°

然后在该角度附近（±15°窗口）查询原始雷达数据中的最近距离。

### 日志证据（失败）

```
[REACT_DYN] 角度 -30.7° 窗口内最小5个距离: [1.72, 1.72, 1.73, 1.74, 1.75], 点数=37
[REACT_DYN] 角度 -43.0° 窗口内最小5个距离: [1.65, 1.65, 1.68, 1.72, 1.75], 点数=36
[REACT_DYN] 角度 -54.3° 窗口内最小5个距离: [1.58, 1.58, 1.61, 1.68, 1.74], 点数=37
```

- 窗口内 **36~38 个有效点**（点数充足，不是无数据）
- 但**所有距离都在 1.58m ~ 1.78m**（远处墙壁）
- **完全没有 0.3m 左右的近距离点**

### 失败原因

**障碍物太小（细柱子/小箱子），雷达光束从障碍物侧面"穿透"到远处墙壁。**

在 −30° ~ −55° 方向上，障碍物的实际宽度很窄（可能只有几厘米），雷达的点云分辨率不够或障碍物正好落在两个点之间，导致窗口内所有点都打在远处的墙上。

### 结论

动态角度跟踪在这种"小障碍物"场景下不可行。需要换方案——**不依赖侧向雷达，仅用前方雷达做主判断**。