...
订阅相机 → BPU 推理 → 保存 JPG + MP4 + CSV 到 ~/dev_ws/log/debug/bpu_test/
无 OpenCV 窗口依赖，可在纯 SSH 下运行。
"""
...
from racing_stage2_param_test.lane_follow import _compute_seg_mask, _center_offset
...
class BpuDirectTest(Node):
    ...
    def cb(self, msg):
        ...
        # 前 10 帧 + 之后每秒一帧保存图片
        save_interval = max(1, int(1.0 / max(0.001, (time.time() - t0))))
