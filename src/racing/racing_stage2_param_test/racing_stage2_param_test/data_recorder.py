...
订阅惯导 + 视觉 + 底盘所有关键话题，以 CSV 格式记录到 log/data_records/。
每次新运行覆盖上一次的 latest_record.csv，方便快速复盘。

记录字段 (CSV 列)：
  timestamp, system_time
  — 位姿 —
  wheel_x, wheel_y, wheel_yaw_deg, wheel_vx, wheel_vy, wheel_wz
...
class DataRecorder(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'数据记录器启动 -> {self._csv_path}  {stamp}  @ {rate_hz}Hz')
    ...
    def _tick(self):
        ...
        if self._row_count % 600 == 0:
            self._csv_file.flush()
            self.get_logger().info(f'已记录 {self._row_count} 行 -> {self._csv_path}')
    ...
    def destroy_node(self):
        ...
        self.get_logger().info(
            f'数据记录结束 -> {self._csv_path}  共 {self._row_count} 行  {stamp}'
        )
        super().destroy_node()
