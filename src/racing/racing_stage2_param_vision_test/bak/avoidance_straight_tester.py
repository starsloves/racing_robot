"""直行避障台架节点：任务编排 + ROS；S1 几何见 s1_geometry。"""
...
from racing_stage2_param_test.s1_geometry import cross_segment_m
...
class AvoidanceStraightTester(DirectInertialTester):
    ...
    状态机见父类 _try_avoid_step()。
    直行 ω=0 不纠航，日志写入 session file。
    """
    ...
    def _load_straight_test_parameters(self) -> None:
        ...
        # 覆盖父类默认值：直行台架用 leg1=0.30 leg2=0.40
    ...
    def _log_ready(self) -> None:
        self._log_info(
            ...
            f'ω转={self._avoid_turn_angular_speed:.2f} 转弯v={self._avoid_turn_linear_speed:.2f} 直行v={self._avoid_leg_linear_speed:.2f}，'
    ...
    def try_start_mission(self):
        ...
        if self.start_after_time is None:
            ...
            if not self.reported_start_delay:
                self.publish_feedback(
                    f'{self.test_feedback_prefix}{self.start_delay_sec:.2f}s 后开始 | '
                    f'yaw={self.format_yaw_deg(self.current_yaw)}deg'
                )
        ...
        self.publish_feedback(
            f'{self.test_feedback_prefix}开始直行 {self.straight_distance_m:.2f}m '
