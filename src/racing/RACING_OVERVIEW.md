...
| 核心文件 | 说明 |
|---|---|
| `stage3_return_navigator.py` | 测试方案主体 |
| `return_track.py` | 路点 YAML 加载器 |
| `global_path_planner.py` | A* 规划 mixin |
| `config/return_track_clockwise.yaml` | 顺时针返程路点 |
| `config/return_stage3.yaml` | 参数配置 |

### 4.1 返程路径

**顺时针** — `config/return_track_clockwise.yaml`：

```
mission_start: (2.38, 3.32)   # 赛道终点
  → (1.83, 3.22) @ 0.16
  → (1.33, 2.77) @ 0.17
  → (0.93, 2.12) @ 0.16
  → (0.55, 1.35) @ 0.14
  → (0.30, 0.70) @ 0.12
  → goal: (0.20, 0.20) @ 0.10, yaw 100.0
```

速度衰减：0.16→0.17→0.16→0.14→0.12→0.10 m/s，末段精确停靠。
...
```
加载 return_track_*.yaml
逐路点 Pure Pursuit 跟踪:
  航向差>70 → 原地转向; 否则 PP 曲率控制
  距目标<tolerance → 下一路点
全部完成 → P 点停靠, 对齐 100.0
```

可选 A*（`use_occupancy_grid_planner=true`）：基于 `/map` occupancy grid，重规划周期 0.25s，动态障碍检测 2.5m。
