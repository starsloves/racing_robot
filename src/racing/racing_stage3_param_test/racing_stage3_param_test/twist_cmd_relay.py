...
from racing_stage3_param_test.cmd_vel_stop import (
...
class TwistCmdRelay(Node):
    def __init__(self):
        super().__init__('stage3_test_cmd_relay')

        self.declare_parameter('input_topic', '/stage3_cmd_vel')
        self.declare_parameter('output_topic', '/cmd_vel')

        self.input_topic = str(self.get_parameter('input_topic').value).strip() or '/stage3_cmd_vel'
        ...
        self.get_logger().info(
            f'阶段三测试速度中继已启动: {self.input_topic} -> {self.output_topic}'
        )
