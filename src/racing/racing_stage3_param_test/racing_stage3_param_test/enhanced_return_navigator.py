...
class EnhancedReturnNavigator(Node):
    ...
    def _start_mission(self):
        ...
        self._publish_feedback(
            f'return started, {len(self.return_waypoints)} waypoints, '
            f'origin=({self.path_origin_pose[0]:.2f},{self.path_origin_pose[1]:.2f}) '
            f'yaw={math.degrees(self.path_origin_yaw):.1f}°'
        )
    ...
    def _local_pose(self):
        ...
        cos_y = math.cos(self.path_origin_yaw)
        sin_y = math.sin(self.path_origin_yaw)
        return (cos_y * dx + sin_y * dy, -sin_y * dx + cos_y * dy)

    def _local_yaw(self):
        if self.current_yaw is None or self.path_origin_yaw is None:
            return None
        return self._normalize_angle(self.current_yaw - self.path_origin_yaw)
    ...
    def _run_pursuit(self):
        ...
        local_yaw = self._local_yaw()
        if local_pose is None or local_yaw is None:
            self.get_logger().warn_throttle(2.0, 'no local pose, waiting for odom')
            return

        self._advance_waypoint(local_pose)
        final_wp = self.return_waypoints[-1]
        final_dist = math.hypot(final_wp['x'] - local_pose[0],
                                final_wp['y'] - local_pose[1])
        ...
        if self.path_index >= len(self.return_waypoints) - 1 and final_dist <= self.goal_tolerance:
            ...
            if target_yaw_deg is not None:
                yaw_err = self._angle_error(math.radians(target_yaw_deg), local_yaw)
        ...
        # ── 追踪当前路点 ──
        wp = self.return_waypoints[self.path_index]
        dx = wp['x'] - local_pose[0]
        dy = wp['y'] - local_pose[1]
        # 转到车体坐标系
        tx = math.cos(local_yaw) * dx + math.sin(local_yaw) * dy
        ty = -math.sin(local_yaw) * dx + math.cos(local_yaw) * dy
