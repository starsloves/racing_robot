...
def _emergency_stop_action():
    return ExecuteProcess(
        cmd=[
            ...
            (
                'set +e; '
                # 先停�?                'ros2 topic pub --once /cmd_vel geometry_msgs/msg/Twist '
                '"{linear: {x: 0.0, y: 0.0, z: 0.0}, angular: {x: 0.0, y: 0.0, z: 0.0}}" '
                '2>/dev/null & '
                # 杀掉所有相关进程（雷达驱动放在最前面�?                'pkill -15 -f lslidar_driver_node 2>/dev/null; '
                'sleep 0.3; '
...
def generate_launch_description():
    ...
    # 视觉相机启动参数（集�?Aurora 930 驱动�?    vision_camera_arg = DeclareLaunchArgument('vision_camera', default_value='true')
    ...
    # map_overlay：提�?/map topic �?map �?odom_combined TF
    ...
    # Aurora 930 相机节点（供视觉车道居中使用�?    aurora_node = Node(
