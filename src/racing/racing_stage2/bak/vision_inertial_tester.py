...
原理�?  - 保留 plan 的段序列（build_ring_plan），知道什么时候该转弯
  - turn 段：开环定时转向，ω=0.75 rad/s
  - move 段：视觉赛道居中 PID，取代航向纠�?  - 避障：激光减�?+ 视觉保持居中
...
class VisionInertialTester(DirectInertialTester):
    ...
    def __init__(self):
        # 缺省值（�?control loop 提前崩溃�?        self.vision_enabled = False
        self.vision_kp = 1.2
        self.vision_kd = 0.4
        self.vision_ki = 0.02
    ...
    def run_turn_segment(self):
        ...
        self.get_logger().info(
            f'[Vision] 开�?{a:+.0f}° {elap:.1f}s/{total:.1f}s',
            throttle_duration_sec=0.5,
        )

        if elap >= total:
            # 完成后更�?current_yaw（开环估计）
