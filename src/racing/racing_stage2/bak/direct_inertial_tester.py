...
class DirectInertialTester(Stage2InertialNavigator):
    def __init__(self):
        ...
        # 转弯障碍检测参�?        self.declare_parameter('turn_obstacle_stop_m', 0.25)
        ...
        self.all_clusters = []  # 缓存当前帧的所有聚�?        self._cluster_window_config = {
            'min_x': 0.30,
            'max_x': 2.50,
            'half_y': 0.50,
            'gap_tolerance': 0.12
        ...
        self.segment_end_pose = None  # 世界坐标系下的段终点（仅 world 模式使用�?        self._world_start_pose = None  # 世界坐标系下的起点信�?
        self.get_logger().info(
            ...
            f'侧边阈�?{_avoid_cfg.side_detour_threshold_m:.2f}m '
            f'闭环转弯 tol={_avoid_cfg.avoider_heading_tolerance_deg:.1f}deg�?
            f'可视化窗�?[{self._cluster_window_config["min_x"]:.2f}-{self._cluster_window_config["max_x"]:.2f}m, '
    ...
    def navigation_yaw(self):
        """统一位姿航向（current_yaw）；轮速模式下�?/odom 写入，IMU 仅诊断�?""
    ...
    def _sync_unified_pose_from_wheel(self) -> None:
        ...
        if self.current_wheel_yaw is None:
            return
        # 位置用轮速里程计，但航向角始终用 IMU（轮�?yaw 不准�?        # self.current_yaw = self.current_wheel_yaw  # 注释掉，改用 IMU yaw

    def imu_callback(self, msg):
        self.imu_yaw = self.quaternion_to_yaw(msg.orientation)
        # 航向角始终使�?IMU，不管位置源是什�?        self.current_yaw = self.imu_yaw
    ...
    def _log_plan_summary(self, nav_succeeded: bool) -> None:
        lines = [f'nav_succeeded={nav_succeeded} 共{len(self.plan)}�?']
    ...
    def _setup_wheel_odom_position(self) -> None:
        """位姿/航向/计程/控制统一用轮�?/odom；EKF/IMU 仅诊断�?""
        ...
        if self._use_wheel_odom_for_distance:
            ...
            self.get_logger().info(
                f'{self.test_feedback_prefix}统一位姿�?{self._navigation_pose_source} '
                f'topic={self._wheel_odom_topic} '
                f'(xy+yaw+计程+控制同源; IMU/EKF {self.odom_topic} 仅日�? '
                f'warmup {self._wheel_odom_warmup_sec:.2f}s×'
                f'{self._wheel_odom_warmup_min_msgs}�?'
            )
    ...
    def _unify_segment_pose(self, segment) -> None:
        """段起�?航向/计程轴与 current_yaw(current_position) 完全对齐�?        
        ...
        if seg_type == 'turn':
            ...
            # 统一使用增量角度 angle_deg，简单可�?            angle_deg = float(segment.get('angle_deg', 0.0))
        ...
        if is_world and 'target' in segment:
            # anchor 用当前位置（段开始时记录�?            if self.current_position is not None:
        ...
        # 相对坐标系：从当前位置开�?        if self.current_position is not None:
            self.segment_start_pose = self.current_position
    ...
    def resolve_test_direction(self, raw_value):
        ...
        if normalized in ('clockwise', 'cw', '顺时�?):
            return 'clockwise'
        if normalized in ('counterclockwise', 'ccw', 'anticlockwise', 'anti-clockwise', '逆时�?):
    ...
    def direction_text(self):
        return '顺时�? if self.test_direction == 'clockwise' else '逆时�?
    ...
    def start_mode_text(self):
        if self.nav_succeeded_for_test_start():
            return '按比赛到达通道口后的惯导入口开�?
        return '按比赛未经过通道口时的完整入环动作开�?
    ...
    def rectangle_segment_label(self, segment):
        ...
        if self.direction == 'clockwise':
            ...
            labels = {
                'rect_enter_align': '通道后起点入口对�?,
                'rect_first_leg': f'底边向左 {d:.2f}m �?,
                'rect_corner_1': '左下拐角',
                'rect_side_1': f'左边向上 {d:.2f}m �?,
                'rect_corner_2': '左上拐角',
                'rect_top': f'顶边向右 {d:.2f}m �?,
                'rect_corner_3': '右上拐角',
                'rect_side_2': f'右边向下 {d:.2f}m �?,
                'rect_corner_4': '右下拐角',
                'rect_return_origin': f'底边回起�?{d:.2f}m �?,
            }
        else:
            d = segment.get('distance_m', 0)
            labels = {
                'rect_enter_align': '通道后起点入口对�?,
                'rect_first_leg': f'底边向右 {d:.2f}m �?,
                'rect_corner_1': '右下拐角',
                'rect_side_1': f'右边向上 {d:.2f}m �?,
                'rect_corner_2': '右上拐角',
                'rect_top': f'顶边向左 {d:.2f}m �?,
                'rect_corner_3': '左上拐角',
                'rect_side_2': f'左边向下 {d:.2f}m �?,
                'rect_corner_4': '左下拐角',
                'rect_return_origin': f'底边回起�?{d:.2f}m �?,
            }
    ...
    def start_segment(self, index):
        ...
        self.last_progress_bucket = -1
        self.active_turn_heading_tolerance = self.heading_tolerance
        self._pure_linear_after_avoid = False  # 新段恢复惯导全控�?        self._avoider.reset()
        ...
        if segment_type == 'turn':
            # 计算实际转弯角度（基�?start_yaw �?target_yaw�?            if hasattr(self, 'segment_start_yaw') and hasattr(self, 'segment_target_yaw') and \
               self.segment_start_yaw is not None and self.segment_target_yaw is not None:
                ...
                self.publish_feedback(
                    f'{self.test_feedback_prefix}当前位置: {label}，开始{turn_text} {abs(actual_turn_deg):.0f} �?
                )
            else:
                angle_deg = float(segment.get('angle_deg', 0.0))
                turn_text = '左转' if angle_deg > 0.0 else '右转'
                self.publish_feedback(
                    f'{self.test_feedback_prefix}当前位置: {label}，开始{turn_text} {abs(angle_deg):.0f} �?
                )
            return
        ...
        if segment_type == 'move':
            ...
            self.publish_feedback(
                f'{self.test_feedback_prefix}当前位置: {label}，目标直�?{distance_m:.2f}m'
            )
            return

        if segment_type == 'pause':
            self.publish_feedback(f'{self.test_feedback_prefix}当前位置: {label}，短暂停�?)
    ...
    def scan_callback(self, msg):
        ...
        # 新增：聚类可视化（前�?0.3-2.5m，左�?±0.5m 窗口�?        try:
    ...
    def run_turn_segment(self):
        ...
        # 转角障碍检测：前方过近 �?蠕行转弯
        # �?turn_obstacle_stop_m(0.25m) 替代 detour_obstacle_distance(0.48m)
    ...
    def finish_mission(self):
        ...
        self.get_logger().info('第二阶段测试完成，自动退�?)
    ...
    def control_loop(self):
        ...
        if (
            ...
            self._log_session(
                'TIMEOUT',
                f'段超�?{desc} | {self._pose_diagnostic()}',
            )
            self.publish_feedback(
                f'{self.test_feedback_prefix}段超�? {desc}'
            )
            self._avoider.reset()
            self.start_segment(self.plan_index + 1)
            return
    ...
    def build_ring_plan(self):
        ...
        # 检查是否返回了 (plan, start_pose) 元组（世界坐标系�?        if isinstance(result, tuple) and len(result) == 2:
    ...
    def try_start_mission(self):
        ...
        if self.start_after_time is None:
            ...
            if not self.reported_start_delay:
                ...
                self.publish_feedback(
                    ...
                    f'{self.start_delay_sec:.2f}s 后开�?
                )
        ...
        self._log_session(
            'MISSION',
            f'任务开�?方向={self.direction_text()} | {self._pose_diagnostic()}',
        )
        self.publish_feedback(
