...
提供:
  - VisionLaneEngine   : �?BPU 推理引擎，可被任意节点复�?  - LaneFollowNode     : 独立 ROS 2 节点，裸视觉居中测试
"""
...
class VisionLaneEngine:
    ...
    def process(self, bgr_image):
        ...
        # 预处�?        rgb = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGB)
        inp = np.expand_dims(
            cv2.resize(rgb, (640, 640)).transpose(2, 0, 1).astype(np.uint8), axis=0
        )

        # BPU 推理（直接传 numpy 数组，无需 pyDNNTensor 包装�?        try:
...
class LaneFollowNode(Node):
    ...
    def __init__(self):
        ...
        # 缓存最近一次推理结�?        self._last_result = None   # dict
