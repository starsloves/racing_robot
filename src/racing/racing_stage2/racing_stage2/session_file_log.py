...
class SessionFileLog:
    ...
    def write(self, line):
        if self._file is None:
            return
        # The vision node appends capture diagnostics to the same S2 log.
        # Re-seek under the shared advisory lock so this persistent writer
        # cannot overwrite lines appended by that node.
