...
```
config/
└── stage1_controller.yaml  # 统一配置文件（包含视觉通道导航参数）
```
...
- 二维码识别触发后退
- 路径记录与后退
- **[NEW] 视觉通道导航（基于YOLOv8-Seg的中线跟随）**
- 地图自由空间区域进入（A* 规划 + Pure Pursuit，作为视觉导航降级方案）
...
- 避障恢复：15个参数（反舵 + 回正航向）
- 后退功能：9个参数
- **[NEW] 视觉通道导航：20个参数（模型配置、控制增益、速度规划、融合策略）**
- 通道导航：50+个参数（路点、控制、Pure Pursuit、A*规划、重定向、左侧恢复）
- Phase切换：6个参数

**视觉导航新增特性：**
- 端口 8081 提供 HTTP 实时预览
- 边界提取 + 中线路径生成
- 多行采样前瞻控制
- 纵向视觉定长估计
- IMU直行兜底机制
